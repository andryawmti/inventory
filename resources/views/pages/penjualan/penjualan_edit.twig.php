...
{% set penjualan = params.penjualan[0] %}
{% set detail_penjualan = params.detail_penjualan %}
{% block main_content %}
<script>
    var prod = {{ detail_penjualan | json_encode |raw }};
    let tot = {{penjualan.total_harga}}
</script>
<!-- START Page content-->
<div class="content-wrapper">
    <h3>Tambah Transaksi Penjualan
        <small>Anda bisa melakukan perubahan data transaksi penjualan dengan mudah pada halaman ini.</small>
    </h3>
    <ol class="breadcrumb">
        <li>
            <a href="/">Dashboard</a>
    ...
    <div class="row">
        <div class="col-md-12">
            <form method="POST" id="updatePenjualan" action="{{ url('/penjualan/') }}/{{penjualan.id_transaksi}}" data-parsley-validate = "" novalidate="" class="form-horizontal">
                <!-- START panel-->
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <input type="text" id="tgl_transaksi" class="form-control date_picker" readonly="readonly" value="{{ penjualan.tgl_transaksi | date('Y-m-d') }}" required="required" class="form-control">
                            </div>
                            ...
                            <div class="col-sm-3">
                                <select id="id_pelanggan" required="required" class="form-control">
                                    ...
                                    <option {%if pelanggan.id_pelanggan == penjualan.id_pelanggan%}selected="selected"{%endif%} value="{{ pelanggan.id_pelanggan }}">
                        ...
                        <div class="table table-bordered table-responsive table-striped">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <td>{{t.harga}}</td>
                                    <td>{{t.harga*t.qty}}</td>
                                    <td><a href="#" id = "{{i}}" onclick="hapusProduk(this)">Hapus</a></td>
                                </tr>
                                {%endfor%}
                                <tr>
                                    <td></td>
                                ...
                                <tr style="background: #0288D1; color: #FFFFFF;">
                                    ...
                                    <td><strong>Total: </strong>&nbsp<strong id="total">{{penjualan.total_harga}}</strong></td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
...
<script type="text/javascript">
    ...
    $('#updatePenjualan').on('submit', function(e){
        if(produks.length <= 0) {
        ...
        let my_url = $('#updatePenjualan').attr('action');
        console.log(my_url);
        $.ajax({
            headers: {
            ...
            dataType: 'JSON',
            data: {
                'produks':produks, 'id_pelanggan':id_pelanggan, 'tgl_transaksi':tgl_transaksi, 'total':total, 'biaya_kirim':biaya_kirim, '_method':'PUT'
            },
            success: function(result,status,xhr){
